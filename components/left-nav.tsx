    ...
    <aside className="menu">
      <ul className="menu-list">
        {props.menuItems.map((menuItem, menuItemIndex) => {
          ...
          if (menuItem.links) {
            elements.push(
              <p key={'label' + menuItemIndex} className="menu-label">{menuItem.label}
              </p>
            );
            menuItem.links.map((link, linkIndex) => {
              elements.push(
                <li key={'link' + linkIndex}><a href={link.slug} className={(router().pathname === link.slug) ? 'is-active' : ''}>{link.title}</a></li>
              );
            });
          }
