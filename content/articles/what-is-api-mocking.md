...
excerpt: Learn what API mocking is, how it works, and how it supports API sandboxing and virtualization for internal and third-party APIs
meta:
  title: Mockoon's guide to API mocking
  description: Learn what API mocking is, how it works, and how it supports API sandboxing and virtualization for internal and third-party APIs
...
Depending on your context, this approach is also referred to as API sandboxing or API virtualization.
...
In practice, API mocking is often the foundation for API sandboxing, and in more advanced setups, part of an API virtualization strategy.
...
## Use cases: why do we need mock APIs, sandboxes, or virtualized APIs?
...
Mocking internal or third-parties APIs helps you accelerate your development lifecycles and establish safe API sandboxes for your teams.
...
Thanks to API mocking, you can start [working with third-party APIs](/case-studies/localazy-speed-development-api-mocking/) in no time. Instead of registering for an account, configuring accesses, and provisioning tokens, you can mock or virtualize the needed API endpoints and start using them in your front-end or back-end applications right away.
...
API mocking gives you the flexibility you need to run complex integration tests for interconnected systems, microservices, or third-party APIs integrations. It allows you to run faster and more comprehensive tests that avoid relying on third-party APIs. With API virtualization capabilities, you can also cover more edge cases: slower response time, random failures, etc.
...
Sometimes you have to make your APIs available to clients or users before they commit to using them. With API mocking and sandboxing, you can provide a simulation of the actual API for testing purposes without the need to give access to your product or provision credentials. Internal or pre-sales demos can benefit from API mocking in the same way.  
...
The variety of tools can give a different kind of mocking, sandboxing, or virtualization experience. Here is a list of the most common differences:

### Online vs local

Among all the API mocking tools available, we can distinguish two approaches. The online or cloud one, where you register on a website and configure a mock API or sandbox using a web interface. Then, the tool exposes the fake API on a subdomain (e.g. temp-api123.mocking-tool.com). The advantage is the immediate availability of the mock for coworkers. The disadvantage is that your mock data resides in a third-party database which may not be an option depending on your industry (banking, health, etc.).  
...
While API mocking is easy to set up, it comes with some challenges. The biggest one is to create accurate enough mock APIs and maintain them up-to-date feature after feature. The same applies to API sandboxes and virtualized endpoints: nothing is more useless than endpoints that are completely outdated.
