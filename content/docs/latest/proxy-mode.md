...
![click on proxy tab](docs-img:open-proxy-options.png)
...
![tick the Proxy checkbox](docs-img:enable-proxy.png)
...
![tick the Proxy no prefix forward checkbox](docs-img:proxy-no-forward.png)

## Proxy headers

Proxy specific headers can also be added, both to the forwarded request and the response received from the target API.

![add proxy headers by filling the keys and values](docs-img:proxy-headers.png)
